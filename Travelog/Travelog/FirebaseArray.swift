...
class FirebaseArray {
  ...
  func removeAtIndex(index: Int) {
    let item = list[index]
    let itemRef = ref.childByAppendingPath(item.key)
    itemRef.removeValue()
  }
  ...
  private func serverMove(snap: FDataSnapshot!, prevKey: String?) {
    let key = snap.key
    let oldPosition = findKeyPosition(key)
    if let oldPosition = oldPosition {
      let data = list[oldPosition]
      list.removeAtIndex(oldPosition)
      let newPosition = moveTo(key, data: data, prevKey: prevKey)
  ...
  private func moveTo(key: String, data: FDataSnapshot, prevKey: String?) -> Int {
    let position = placeRecord(key, prevKey: prevKey)
